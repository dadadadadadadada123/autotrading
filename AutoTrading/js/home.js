...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const homeRankingShell = document.getElementById('homeRankingShell');
    const homeRankingView = document.getElementById('homeRankingView');
    const homeNewsView = document.getElementById('homeNewsView');
    const homeSidebarNavs = Array.from(document.querySelectorAll('[data-home-nav]'));
    const homeNewsList = document.getElementById('homeNewsList');
    const homeNewsStatus = document.getElementById('homeNewsStatus');
    const homeNewsSubtitle = document.getElementById('homeNewsSubtitle');
    const homeNewsRefresh = document.getElementById('homeNewsRefresh');
    const homeNewsFilters = Array.from(document.querySelectorAll('[data-news-query]'));
    const homeNewsSearchForm = document.getElementById('homeNewsSearchForm');
    const homeNewsSearch = document.getElementById('homeNewsSearch');
    const homeNewsSearchClear = document.getElementById('homeNewsSearchClear');
    ...
    const setActiveSidebarNav = (view) => {
        homeSidebarNavs.forEach((nav) => {
            const isActive = nav.dataset.homeNav === view;
            nav.classList.toggle('is-active', isActive);
            nav.classList.toggle('text-emerald-400', isActive);
            nav.classList.toggle('bg-slate-700/50', isActive);
            nav.classList.toggle('text-slate-400', !isActive);
        });
    };

    const clearRankingTabActive = () => {
    ...
    const setActiveRankingTab = (type) => {
        ...
        watchlistApi?.exitWatchlistMode();
    };
    ...
    const setHomeView = (view) => {
        ...
        homeRankingShell?.classList.toggle('is-hidden', isNews);
        homeNewsView?.classList.toggle('is-hidden', !isNews);
        setActiveSidebarNav(view);

        if (!isNews) {
            setActiveRankingTab(activeRankingType);
        }
    };

    const clearNewsFilterActive = () => {
        homeNewsFilters.forEach((filter) => filter.classList.remove('is-active'));
    };

    const setNewsFilterActive = (query) => {
        homeNewsFilters.forEach((filter) => {
            filter.classList.toggle('is-active', filter.dataset.newsQuery === query);
        });
    };

    const updateNewsSearchClearButton = () => {
        if (!homeNewsSearchClear || !homeNewsSearch) return;
        homeNewsSearchClear.classList.toggle('show', Boolean(homeNewsSearch.value.trim()));
    };

    const getNewsDisplayLabel = (query, searchText) => {
    ...
    const openNewsView = () => {
        setHomeView('news');
        if (activeNewsSearch) {
            clearNewsFilterActive();
            loadNews(activeNewsSearch);
            return;
        }

        setNewsFilterActive(activeNewsQuery);
        loadNews(activeNewsQuery);
    };
    ...
    const searchNews = (keyword) => {
        ...
        if (!text) {
            activeNewsSearch = '';
            if (homeNewsSearch) homeNewsSearch.value = '';
        ...
        activeNewsSearch = text;
        activeNewsQuery = text;
        clearNewsFilterActive();
        updateNewsSearchClearButton();
        loadNews(text);
    };

    const setNewsStatus = (message = '', show = Boolean(message)) => {
        if (!homeNewsStatus) return;
        homeNewsStatus.textContent = message;
        homeNewsStatus.classList.toggle('show', show);
    };

    const filterNewsItemsByKeyword = (items, keyword) => {
        ...
        return items.filter((item) => {
            ...
            return tokens.every((token) => text.includes(token) || lowered.includes(token.toLowerCase()));
        });
    };

    const highlightNewsText = (text, keyword) => {
        ...
        tokens.forEach((token) => {
            ...
            result = result.replace(regex, '<mark class="home-news-highlight">$1</mark>');
        });
        return result;
    };

    const renderNewsItems = (items = [], keyword = '') => {
        if (!homeNewsList) return;

        const filteredItems = filterNewsItemsByKeyword(items, keyword);

        if (!filteredItems.length) {
            const emptyMessage = keyword
                ? `"${escapeHtml(keyword)}"이(가) 포함된 뉴스가 없습니다.`
                : '표시할 뉴스가 없습니다.';
            homeNewsList.innerHTML = `<div class="home-news-empty">${emptyMessage}</div>`;
            setNewsStatus('', false);
            return;
        }

        setNewsStatus('', false);
        homeNewsList.innerHTML = filteredItems.map((item) => `
            <article class="home-news-card">
                <h3>
                    <a href="${escapeHtml(item.link)}" target="_blank" rel="noopener noreferrer">
                        ${highlightNewsText(item.title, keyword)}
                    </a>
                </h3>
                ${item.description ? `<p>${highlightNewsText(item.description, keyword)}</p>` : ''}
                <div class="home-news-card-meta">
                    <span>${escapeHtml(item.stock || '증시')}</span>
                    <time datetime="${escapeHtml(item.pubDate)}">${escapeHtml(item.pubDate || '')}</time>
                </div>
            </article>
        `).join('');
    };

    const loadNews = async (query = activeNewsQuery) => {
        if (!homeNewsList) return;

        const requestQuery = String(query ?? '').trim();
        activeNewsQuery = requestQuery;
        homeNewsList.replaceChildren();
        setNewsStatus('뉴스를 불러오는 중...', true);

        if (homeNewsSubtitle) {
            const label = getNewsDisplayLabel(requestQuery, activeNewsSearch);
            homeNewsSubtitle.textContent = activeNewsSearch
                ? `"${activeNewsSearch}" 검색 결과`
                : `${label} 관련 최신 뉴스`;
        }
        ...
        try {
            const params = new URLSearchParams({ limit: activeNewsSearch ? '50' : '30' });
            if (requestQuery) params.set('q', requestQuery);

            const response = await authFetch(`/api/news?${params.toString()}`, {
                cache: 'no-store',
            ...
            const keyword = activeNewsSearch || '';
            renderNewsItems(payload.items || [], keyword);
        } catch (error) {
            if (error.name === 'AbortError') return;
            console.error('News request failed.', error);
            homeNewsList.innerHTML = '<div class="home-news-empty">뉴스를 불러오지 못했습니다.</div>';
            setNewsStatus(error.message || '뉴스를 불러오지 못했습니다.', true);
        }
    };
    ...
    if (searchBar && searchModal && searchResults) {
        ...
        searchBar.addEventListener('keydown', (event) => {
            ...
            const stock = selected || { code: keyword, name: keyword };

            if (watchlistApi?.isWatchlistMode() && watchlistApi.getActiveWatchlistGroup()?.type === 'manual') {
                watchlistApi.addStockToActiveManualGroup(stock).then((added) => {
                    if (added) searchModal.classList.remove('show');
                });
                return;
            }

            openTradingPage(stock.code || keyword);
        });
        ...
        searchResults.addEventListener('click', (event) => {
            ...
            const stock = latestResults[activeSearchIndex] || {
                code: button.dataset.code,
                name: button.querySelector('.search-result-name')?.textContent?.trim() || '',
            };
    ...
    homeSidebarNavs.forEach((nav) => {
        nav.addEventListener('click', () => {
            const view = nav.dataset.homeNav || 'ranking';
            if (view === 'news') {
                openNewsView();
                return;
            }
    ...
    homeNewsFilters.forEach((filter) => {
        filter.addEventListener('click', () => {
            const query = filter.dataset.newsQuery || '';
            activeNewsSearch = '';
            if (homeNewsSearch) homeNewsSearch.value = '';
            updateNewsSearchClearButton();
            setNewsFilterActive(query);
            if (activeHomeView !== 'news') {
                openNewsView();
            }
            loadNews(query);
        });
    ...
    homeNewsSearchForm?.addEventListener('submit', (event) => {
        event.preventDefault();
        clearTimeout(newsSearchTimer);
        searchNews(homeNewsSearch?.value || '');
    });

    homeNewsSearch?.addEventListener('input', () => {
        updateNewsSearchClearButton();
        clearTimeout(newsSearchTimer);

        const keyword = homeNewsSearch.value.trim();
        if (!keyword) {
            newsSearchTimer = setTimeout(() => searchNews(''), 300);
            return;
        }

        newsSearchTimer = setTimeout(() => searchNews(keyword), 400);
    });

    homeNewsSearchClear?.addEventListener('mousedown', (event) => {
        event.preventDefault();
    });

    homeNewsSearchClear?.addEventListener('click', () => {
        clearTimeout(newsSearchTimer);
        searchNews('');
        homeNewsSearch?.focus();
    });

    homeNewsRefresh?.addEventListener('click', () => {
        if (activeNewsSearch) {
            loadNews(activeNewsSearch);
            return;
        }
    ...
    rankingRefresh?.addEventListener('click', () => {
        if (watchlistApi?.isWatchlistMode()) {
            watchlistApi.reloadActiveWatchlist();
            return;
        }
        loadRanking(activeRankingType);
    });
    ...
    rankingList?.addEventListener('click', (event) => {
        if (watchlistApi?.isWatchlistMode()) return;
        const card = event.target.closest('.home-ranking-card[data-target]');
        if (!card) return;
        openTradingPage(card.dataset.target);
    });

    watchlistApi = initWatchlistUI({
        authFetch,
        escapeHtml,
        formatNumber,
        openTradingPage,
